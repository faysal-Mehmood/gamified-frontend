...
const Circle = ({ fitBackColor, currentSlide, index }) => {
  return (
    <div className={`w-fit h-fit relative  `}>
      {/* Top circle */}
      <div
        className={`absolute z-1 rounded-full border-2 border-brand-blue 
      bg-circle-gradient ${
      ...
      ${
        currentSlide === index
          ...
          : `w-[82px] h-[82px] left-[6px] top-[173px] transition-all duration-500 ease-in-out`
      }`}
        style={{ position: "absolute" }}
      />

      <div
        className={`${"w-[451px] relative z-10 md:w-[752px] h-[451px] md:h-[752px]"} flex flex-col justify-start gap-3 md:gap-[46px] items-center rounded-full pt-[53px] md:pt-[70px] px-20 md:px-[129px] pb-16 md:pb-[170px] border-2 blur-bg border-brand-blue flex-shrink-0 z-50 upper-index-layer
         bg-circle-gradient
        `}
        style={{
